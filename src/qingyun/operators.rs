...
impl 冰雪清韵操作 {
    ...
    fn 移动韵母(&self, 决策: &mut 冰雪清韵决策) -> 冰雪清韵决策变化 {
        ...
        let (韵母, 安排列表) = self
            ...
            .choose(&mut rng)
            .unwrap();
        决策.元素[韵母] = 安排列表.iter().choose(&mut rng).unwrap().安排;
        冰雪清韵决策变化::无变化()
    }
        ...
        while !队列.is_empty() {
            ...
            for 条件安排 in &self.决策空间.元素[元素] {
                if 决策.允许(条件安排) {
                    ...
                    if !matches!(条件安排.安排, 元素安排::键位韵母 { .. }) {
                        新安排列表.push(条件安排.安排.clone());
                    }
                }
            }
            if !合法 {
                if 新安排列表.is_empty() || matches!(决策.元素[元素], 元素安排::键位韵母 { .. })
                {
                    ...
                    if 决策.元素[元素] == 元素安排::未选取 || 安排 == &元素安排::未选取
                    {
                        变化.拆分改变 = true;
                    }
    ...
    fn 产生副根(&self, 决策: &mut 冰雪清韵决策) -> 冰雪清韵决策变化 {
        ...
        for 字根 in &self.决策空间.字根 {
            ...
            for 条件安排 in &self.决策空间.元素[*字根] {
                if matches!(条件安排.安排, 元素安排::未选取 | 元素安排::键位韵母 { .. })
                {
                    continue;
                }
                if 决策.允许(条件安排) {
    ...
    fn 湮灭副根(&self, 决策: &mut 冰雪清韵决策) -> 冰雪清韵决策变化 {
        ...
        for 字根 in &self.决策空间.字根 {
            let 安排 = &决策.元素[*字根];
            if matches!(安排, 元素安排::未选取 | 元素安排::键位韵母 { .. }) {
                continue;
            }
            for 条件安排 in &self.决策空间.元素[*字根] {
    ...
    fn 交换主副根(&self, 决策: &mut 冰雪清韵决策) -> 冰雪清韵决策变化 {
        ...
        for 字根 in &self.决策空间.字根 {
            ...
            let 乱序安排 = &self.决策空间.元素[*字根]
                .iter()
                .filter(|&x| matches!(x.安排, 元素安排::键位韵母 { .. }))
                .choose(&mut rng);
        ...
        let 当前该键位上主根 = self
            ...
            .find(|&字根| {
                !self.笔画列表.contains(字根)
                    && matches!(决策.元素[*字根], 元素安排::键位韵母 { 键位: k, .. } if k == 键位)
            })
            .unwrap()
            .clone();
        决策.元素[字根] = 安排;
    ...
    fn 交换主根(&self, 决策: &mut 冰雪清韵决策) -> 冰雪清韵决策变化 {
        ...
        let 主根列表: Vec<_> = self
            ...
            .filter_map(|k| {
                ...
                if let 元素安排::键位韵母 { 键位, 韵母 } = &决策.元素[*k] {
