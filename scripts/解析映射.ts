...
const mapping: Record<
	string,
	string | { element: string } | (string | { element: string; index: number })[]
> = data.form.mapping;

// const mapper = ["", "一", "丨", "丶", "丿", "乚", "乙", "〇"];
// for (const key of Object.keys(mapping)) {
//   if (/\d/.test(key)) {
//     const newKey = key.replace(/\d/g, digit => mapper[parseInt(digit, 10)]);
...
function resolve(key: string): string {
  ...
  console.log(`Resolving ${key} -> ${JSON.stringify(val)}`);
	let result: string;
	if (Array.isArray(val)) {
		result = val.map(item => typeof item === "object" ? resolve(item.element)[item.index] : item).join("");
	} else if (typeof val === "object") {
		result = resolve(val.element);
	} else {
		result = val;
	}
...
for (const [key, val] of Object.entries(mapping)) {
	if (!isAnchor(key) && typeof val === "object" && !Array.isArray(val)) {
		const target = val.element;
		if (!isAnchor(target)) {
...
const categories = {
	声: {} as Record<string, string>,
	韵: {} as Record<string, string>,
	形: {} as Record<string, string>,
	特殊: {} as Record<string, string>,
};
...
for (const cat of Object.keys(categories)) {
  const _cat = cat as keyof typeof categories;
  const sortedEntries = Object.entries(categories[_cat]).sort((a, b) => a[1].localeCompare(b[1]));
  categories[_cat] = Object.fromEntries(sortedEntries);
}
